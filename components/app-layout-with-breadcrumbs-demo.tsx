"use client";

import {ChartBar, Folder, Gear, House} from "@gravity-ui/icons";
import {Breadcrumbs} from "@heroui/react";
import {AppLayout, Navbar, Sidebar} from "@heroui-pro/react";

const navItems = [
  {href: "#", icon: <House />, label: "Home"},
  {href: "#", icon: <ChartBar />, label: "Analytics"},
  {href: "#", icon: <Folder />, label: "Projects"},
  {href: "#", icon: <Gear />, label: "Settings"},
];

function AppLayoutWithBreadcrumbsSidebarNav() {
  return (
    <Sidebar.Content>
      <Sidebar.Main>
        <Sidebar.Menu>
          {navItems.map((item) => (
            <Sidebar.MenuItem key={item.label} href={item.href} textValue={item.label}>
              <Sidebar.MenuIcon>{item.icon}</Sidebar.MenuIcon>
              <Sidebar.MenuLabel>{item.label}</Sidebar.MenuLabel>
            </Sidebar.MenuItem>
          ))}
        </Sidebar.Menu>
      </Sidebar.Main>
    </Sidebar.Content>
  );
}

function AppLayoutWithBreadcrumbsSidebarHeader() {
  return (
    <Sidebar.Header>
      <div className="px-2 text-sm font-bold">HeroUI Pro</div>
    </Sidebar.Header>
  );
}

export default function AppLayoutWithBreadcrumbsDemo() {
  return (
    <div className="h-[550px] w-full overflow-hidden rounded-xl border">
      <AppLayout
        sidebarCollapsible="icon"
        sidebar={
          <>
            <Sidebar>
              <AppLayoutWithBreadcrumbsSidebarHeader />
              <AppLayoutWithBreadcrumbsSidebarNav />
            </Sidebar>
            <Sidebar.Mobile>
              <AppLayoutWithBreadcrumbsSidebarHeader />
              <AppLayoutWithBreadcrumbsSidebarNav />
            </Sidebar.Mobile>
          </>
        }
        navbar={
          <Navbar maxWidth="full">
            <Navbar.Header>
              <AppLayout.MenuToggle />
              <Sidebar.Trigger />
              <Navbar.Separator />
              <Breadcrumbs>
                <Breadcrumbs.Item href="#">Projects</Breadcrumbs.Item>
                <Breadcrumbs.Item href="#">Frontend</Breadcrumbs.Item>
                <Breadcrumbs.Item>Tasks</Breadcrumbs.Item>
              </Breadcrumbs>
              <Navbar.Spacer />
            </Navbar.Header>
          </Navbar>
        }
      >
        <div className="p-6">
          <h2 className="text-xl font-bold">With Breadcrumbs</h2>
          <p className="mt-2 text-sm text-foreground/60">
            A project-management-style navbar with a multi-level breadcrumb trail reflecting the
            current navigation depth.
          </p>
        </div>
      </AppLayout>
    </div>
  );
}
