...
export default function HoverCardPlacementsDemo() {
  return (
    <div className="grid gap-4 px-10 py-8">
      <div className="my-2 text-sm text-default-500">Hover buttons</div>

      {(["top", "left", "right", "bottom"] as const).map((placement) => (
        <HoverCard key={placement}>
          <HoverCard.Trigger>
            <Button className="w-full capitalize" variant="bordered">
              {placement}
            </Button>
          </HoverCard.Trigger>
