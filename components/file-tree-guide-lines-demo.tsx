...
export default function FileTreeGuideLinesDemo() {
  return (
    <div className="flex items-start gap-6">
      {([true, "hover", false] as const).map((variant) => (
        <div key={String(variant)} className="flex flex-col gap-2">
          <span className="text-muted text-xs">
            {variant === true ? "always" : variant === false ? "none" : "hover"}
          </span>
          <IconsTree className="w-[260px]" showGuideLines={variant} />
        </div>
      ))}
    </div>
  );
