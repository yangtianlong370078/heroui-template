...
const initialEmployees: Employee[] = Array.from({length: 8}, (_, i) => {
...
export default function ActionBarWithDataGridDemo() {
  const [employees, setEmployees] = useState(initialEmployees);
  const [selectedKeys, setSelectedKeys] = useState<Selection>(new Set());

  const selectionCount =
    selectedKeys === "all" ? employees.length : (selectedKeys as Set<string | number>).size;

  const selectedIds =
    selectedKeys === "all" ? new Set(employees.map((e) => e.id)) : (selectedKeys as Set<number>);
  ...
  const handleExport = useCallback(() => {
    ...
    const csv = [
      ...
      ...selected.map((e) => `${e.name},${e.email},${e.department},${e.status},${e.joinDate}`),
    ].join("\n");
  ...
  const columns: DataGridColumn<Employee>[] = [
    ...
    },
    {
      ...
      allowsSorting: true,
      cell: (item) => (
        <span className="text-muted text-sm tabular-nums">{formatDate(item.joinDate)}</span>
      ),
      header: "Joined",
      id: "joinDate",
    },
    ...
    <div className="w-full max-w-4xl">
      ...
      <ActionBar aria-label="Bulk actions" isOpen={selectionCount > 0}>
        ...
        <ActionBar.Content>
          ...
          <Button aria-label="Export" size="sm" variant="ghost" onPress={handleExport}>
            <ArrowDownToLine />
